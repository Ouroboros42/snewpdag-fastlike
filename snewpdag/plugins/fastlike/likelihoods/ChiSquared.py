...
class ChiSquared(LikelihoodBase):
    def __init__(self, uniform_var: bool = False, **kwargs):
        self.uniform_var = uniform_var
    ...
    def log_likelihood(self, hist_2: np.ndarray[float], hist_1: np.ndarray[float], detectors: DetectorRelation) -> float:
        ...
        chi_squared = (norm_hist_1 - norm_hist_2) ** 2

        if not self.uniform_var:
            chi_squared /= (norm_hist_1 + norm_hist_2)
